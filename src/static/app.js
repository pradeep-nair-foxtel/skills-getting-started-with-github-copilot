  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Debug: Log the activity data to see structure
        console.log(`Activity: ${name}`, details);
        console.log(`Participants:`, details.participants);

        // Create participants list HTML with more robust checking
        let participantsList;
        if (details.participants && Array.isArray(details.participants) && details.participants.length > 0) {
          participantsList = `<ul class="participants-list">
               ${details.participants.map(participant => `<li>
